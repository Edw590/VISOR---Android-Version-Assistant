			...
			switch (intent.getAction()) {
				...
				case (BluetoothDevice.ACTION_FOUND): {
					...
					nearby_devices_bt.add(new ExtDeviceObj(
							...
							address,
							time_detection,
							(int) UtilsSWA.getRealDistanceRSSILOCATIONRELATIVE((int) rssi, UtilsSWA.DEFAULT_TX_POWER),
				...
				case (WifiManager.RSSI_CHANGED_ACTION): {
					UtilsRegistry.setValue(ValuesRegistry.Keys.DIST_ROUTER, UtilsSWA.
							getRealDistanceRSSILOCATIONRELATIVE(intent.getIntExtra(WifiManager.EXTRA_NEW_RSSI, -1),
					...
					if (UtilsPermsAuths.checkSelfPermission(Manifest.permission.ACCESS_FINE_LOCATION)) {
						...
						for (final ScanResult scanResult : wifiManager.getScanResults()) {
							...
							if (ScanResult.UNSPECIFIED == scanResult.distanceCm) {
								distance = (int) UtilsSWA.getRealDistanceRSSILOCATIONRELATIVE(scanResult.level,
										UtilsSWA.DEFAULT_TX_POWER);
							} else {
